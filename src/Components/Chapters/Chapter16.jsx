...
const chapterData = allChaptersData.find(ch => ch.chapter === 16);

const Chapter16 = ({ targetVerseId }) => {
  const [translationsVisible, setTranslationsVisible] = useState(true);
  const [currentVerseIndex, setCurrentVerseIndex] = useState(0);

  // Effect to handle scrolling/highlighting when a verse search occurs
  useEffect(() => {
  ...
  // If chapter data isn't found, show an error
  if (!chapterData) {
    return <div>Chapter 16 data not found. Please update gita-data.js</div>;
  }
