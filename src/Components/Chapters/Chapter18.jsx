...
import './Chapter1.css'; // Reuse the same CSS

// Import the data from the central file
import { allChaptersData } from '../../gita-data.js';
const chapterData = allChaptersData.find(ch => ch.chapter === 18);

const Chapter18 = ({ targetVerseId }) => {
  const [translationsVisible, setTranslationsVisible] = useState(true);
  const [currentVerseIndex, setCurrentVerseIndex] = useState(0);

  // Effect to handle scrolling/highlighting when navigating to a searched verse
  useEffect(() => {
  ...
  // If chapter data isn't found, show a friendly error
  if (!chapterData) {
    return <div>Chapter 18 data not found. Please update gita-data.js</div>;
  }
