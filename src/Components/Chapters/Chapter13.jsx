...
import './Chapter1.css'; // Reuse the same CSS file

// Import the data from the central file
import { allChaptersData } from '../../gita-data.js';
const chapterData = allChaptersData.find(ch => ch.chapter === 13);

const Chapter13 = ({ targetVerseId }) => {
  const [translationsVisible, setTranslationsVisible] = useState(true);
  const [currentVerseIndex, setCurrentVerseIndex] = useState(0);

  // Effect to handle scrolling/highlighting on searched verse
  useEffect(() => {
  ...
  // If chapter data is missing
  if (!chapterData) {
    return <div>Chapter 13 data not found. Please update gita-data.js</div>;
  }
