...
const chapterData = allChaptersData.find(ch => ch.chapter === 17);

const Chapter17 = ({ targetVerseId }) => {
  const [translationsVisible, setTranslationsVisible] = useState(true);
  const [currentVerseIndex, setCurrentVerseIndex] = useState(0);

  // Effect to handle scrolling and highlighting on searched verse
  useEffect(() => {
  ...
  if (!chapterData) {
    return <div>Chapter 17 data not found. Please update gita-data.js</div>;
  }
